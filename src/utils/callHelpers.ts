...
export const stake = async (masterChefContract, pid, amount, account) => {
  if (pid === 0) {
    const tx = await masterChefContract.enterStaking(new BigNumber(amount).times(DEFAULT_TOKEN_DECIMAL).toString(), {
      from: account,
      gasLimit: 500000,
    })
    const receipt = await tx.wait()
  ...
  const tx = await masterChefContract.deposit(pid, new BigNumber(amount).times(DEFAULT_TOKEN_DECIMAL).toString(), {
    from: account,
...
export const unstake = async (masterChefContract, pid, amount, account) => {
  if (pid === 0) {
    const tx = await masterChefContract.leaveStaking(new BigNumber(amount).times(DEFAULT_TOKEN_DECIMAL).toString(), {
      from: account,
      gasLimit: DEFAULT_GAS_LIMIT,
    })
    const receipt = await tx.wait()
  ...
  const tx = await masterChefContract.withdraw(pid, new BigNumber(amount).times(DEFAULT_TOKEN_DECIMAL).toString(), {
    from: account,
    gasLimit: DEFAULT_GAS_LIMIT,
  })
  const receipt = await tx.wait()
...
export const harvest = async (masterChefContract, pid, account) => {
  if (pid === 0) {
    const tx = await masterChefContract.leaveStaking('0', { from: account, gasLimit: DEFAULT_GAS_LIMIT })
    const receipt = await tx.wait()
    return receipt.status
  }

  const tx = await masterChefContract.deposit(pid, '0', { from: account, gasLimit: DEFAULT_GAS_LIMIT })
  const receipt = await tx.wait()
  return receipt.status
}

export const soushHarvest = async (sousChefContract, account) => {
  const tx = await sousChefContract.deposit('0', { from: account, gasLimit: DEFAULT_GAS_LIMIT })
  const receipt = await tx.wait()
  return receipt.status
}

export const soushHarvestHt = async (sousChefContract, account) => {
  const tx = await sousChefContract.deposit({ from: account, gasLimit: DEFAULT_GAS_LIMIT, value: BIG_ZERO })
  const receipt = await tx.wait()
  return receipt.status
}
 ...
 */
export const getUserStakeInMakiHtLp = async (account: string) => {
  try {
 ...
 */
export const getUserStakeInMakiPool = async (account: string) => {
  try {
    const masterContract = getMasterchefContract()
