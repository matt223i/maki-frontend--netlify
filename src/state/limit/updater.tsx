...
import { limitStart, limitRestart, updateStatus, addOrder, limitUpdateFee, limitUpdateGas } from './actions'
import { findOrdersByAddressFromTokens, parseEvent } from '.'
...
export default function LimitUpdater() {
  ...
  const { status, feeExecutor, feeStake } = useSelector<AppState, AppState['limit']>((s) => s.limit)
  const gas = useHecoGasEstimative(30000)

  const handlerRemOrder = useCallback(
    (...events) => {
  ...
  useEffect(() => {
    if (contract && (status === 'idle' || status === 'failed')) {
      ...
      Promise.all([contract.STAKE_FEE(), contract.EXECUTOR_FEE()])
        .then((result) => {
          dispatch(
            limitUpdateFee({
              status: 'succeeded',
