...
export default function PriceInput({ id, label, value, currency, onChange }: PriceInputProps) {
  ...
  return (
    <Container hideInput={false}>
      <InputWrap flexGrow={currency?.symbol ? 0 : 1}>
        {label && <LabelRow htmlFor={id}>{label}</LabelRow>}
        <InputRow selected={false}>
          <NumericalInput value={value} onUserInput={onChange} />
        </InputRow>
      </InputWrap>
