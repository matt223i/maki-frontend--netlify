...
import { Text, Flex } from 'maki-toolkit-v2'
  ...
  const chartOptions = {
    ...
    timeScale: {
      ...
      tickMarkFormatter: (time: BusinessDay | UTCTimestamp, tickMarkType: TickMarkType) => {
        ...
        if (tickMarkType === TickMarkType.Month) return month
        if (tickMarkType === TickMarkType.DayOfMonth) return day
  ...
  useEffect(() => {
    ...
    ;(async () => {
      if (pairAddress !== '') {
        ...
        const timeType =
          candlePeriod === CandlePeriod.FiveMinutes || candlePeriod === CandlePeriod.FifteenMinutes
            ? 'minute'
            : candlePeriod === CandlePeriod.OneHour || candlePeriod === CandlePeriod.FourHours
            ? 'hour'
            : candlePeriod === CandlePeriod.OneDay
            ? 'day'
            : 'week'
        const startType =
          candlePeriod === CandlePeriod.FiveMinutes || candlePeriod === CandlePeriod.FifteenMinutes
            ...
            : 'hour'
        const timeAmount =
          candlePeriod === CandlePeriod.FiveMinutes
            ? 5
            : candlePeriod === CandlePeriod.FifteenMinutes
            ? 15
            : candlePeriod === CandlePeriod.FourHours
            ? 4
            : 1
        const startTime = currentTime.subtract(timeAmount, timeType).startOf(startType).unix()
        const pairChartData = await getHourlyRateData(
        ...
        const candleData: NumericalCandlestickDatum[] = pairChartData.map((item: any) => {
          return {
    ...
    <div style={{ padding: '1rem 2rem' }}>
      <FlexColumnWrapper>
        <ChartHeaderWrapper>
          ...
          {inputCurrency && outputCurrency ? <Text style={{ margin: '0 1rem' }}>/</Text> : <div />}
        ...
        {!hasData && (
          <Text fontSize="14px" textAlign="center">
            Unforunately, this pair doesn&rsquo;t have enough data.
          </Text>
        )}
      </FlexColumnWrapper>
    </div>
