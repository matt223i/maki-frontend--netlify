...
class ChartWrapper extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    const {
      onCrosshairMove: onCrosshairMovePrev,
      onTimeRangeMove: onTimeRangeMovePrev,
      onClick: onClickPrev,
      options: optionsPrev,
      darkTheme: darkThemePrev,
      candlestickSeries: candlestickSeriesPrev,
      lineSeries: lineSeriesPrev,
      areaSeries: areaSeriesPrev,
      barSeries: barSeriesPrev,
      histogramSeries: histogramSeriesPrev,
      from: fromPrev,
      to: toPrev,
    } = prevProps
    if (!autoWidth && !autoHeight) window.removeEventListener('resize', this.resizeHandler)
    if (!equal([onCrosshairMovePrev, onTimeRangeMovePrev, onClickPrev], [onCrosshairMove, onTimeRangeMove, onClick]))
    ...
    if (
      !equal(
        ...
        [options, darkThemeCurrent, candlestickSeries, lineSeries, areaSeries, barSeries, histogramSeries],
      )
    ) {
      this.removeSeries()
      this.handleUpdateChart()
  ...
  resizeHandler = () => {
    ...
    const height =
      autoHeight && this.chartDiv.current ? this.chartDiv.current.parentNode.clientHeight : heightProp || 500
  ...
  handleEvents = () => {
    const chart = this.chart
    const props = this.props
    if (props.onClick) chart.subscribeClick(props.onClick)
    if (props.onCrosshairMove) chart.subscribeCrosshairMove(props.onCrosshairMove)
    if (props.onTimeRangeMove) chart.timeScale().subscribeVisibleTimeRangeChange(props.onTimeRangeMove)
  ...
  handleTimeRange = () => {
    ...
    if (from && to) this.chart.timeScale().setVisibleRange({ from, to })
  }
